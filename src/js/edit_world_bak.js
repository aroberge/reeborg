...
RUR.edit_world.select = function (arg) {
    $("#cmd-result").html(arg);
};
...
RUR.edit_world.add_robot = function (x, y, orientation, tokens){
    "use strict";
    var r, robot = {};
    r = RUR.world.robots[0];
    if (r) {
        r.x = x || 1;
        r.y = y || 1;
        r.orientation = orientation || 0;
        r.tokens = tokens || 0;
    } else {
        robot.x = x || 1;
        robot.y = y || 1;
        ...
        RUR.world.robots = [robot];
    }

    RUR.edit_world.update("added or changed robot", x, y);
