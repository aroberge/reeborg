...
_usage_ = '''
    <h2>Basic commands for Python only</h2>
      <ul>
        <li><code>at_goal()</code>: condition which is "true" if Reeborg has reached "home"</li>
        <li><code>at_goal_orientation()</code>: condition which is "true" if Reeborg has the required orienation </li>
        ...
        <li><code>is_facing_north()</code>: "true" if Reeborg is facing North (towards the top of the screen)</li>
        ...
        <li><code>object_here()</code>: "false" if there is no object at Reeborg's location, otherwise it returns
        the name of the object, <code>"triangle"</code>, <code>"square"</code> or <code>"star"</code>, or the number
        of tokens at that location. </li>
      ...
      <p>To create additional robots, one can use <code>UsedRobot()</code></p>
      <p>In addition, the following commands are available and are not tied to
        any robot in particular.</p>
      <ul>
        <li><code>from my_lib import *</code>: for <b>Python</b>, includes the code found in the Library tab</li>
        ...
        <li><code>sound("true")</code>: turns sound on. Can be turned off by calling with no argument or with "false".
                  Off by default or if delay between actions is less than 250ms.</li>
        <li><code>think(number)</code>: sets the amount of time in milliseconds between each robot action.</li>
        <li><code>write(string)</code>: write a string in Reeborg's Diary <b>only</b> if world is visible - otherwise,
          use <code>print</code>.</li>
        <li><code>select_world(string)</code>: selects a given world.</li>
            <li><code>select_challenge(string)</code>: selects a given (hiddent) world.</li>
        <li><code>print(string)</code>: only valid with Python.  Somewhat equivalent to <code>write</code>.</li>
        <li><code>say(string)</code>: A dialog appears with Reeborg "saying" a message written as a string.</li>
      </ul>
'''
RUR = window.RUR
RUR.ui.add_help(_usage_, "en")
