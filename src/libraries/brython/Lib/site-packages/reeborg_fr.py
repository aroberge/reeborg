...
_usage_ = '''
<h2>Commandes de base pour Python seulement</h2>

  <ul>
    <li><code>au_but()</code>: la condition est "vrai" si Reeborg a atteint son but.</li>
    <li><code>au_but_orientation()</code>: la condition est "vrai" si Reeborg a la bonne orientation.</li>
    ...
    <li><code>face_au_nord()</code>: "vrai" si Reeborg fait face au nord (vers le haut de l'écran).</li>
    <li><code>avance()</code></li>
    <li><code>depose()</code> ou <code>depose("objet")</code> où "objet" est l'un de <code>"jeton"</code>,
      <code>"triangle"</code>, <code>"carré"</code> ou <code>"étoile"</code>: Reeborg dépose l'objet
      en question là où il se trouve.</li>
      <li><code>objet_ici()</code>: "false" s'il n'y a aucun objet à cet endroit, autrement on retourne
      le nom de l'objet.</li>
    <li><code>rien_a_droite()</code>: "vrai" s'il n'y a pas de mur immédiatement à la droite de Reeborg.</li>
    <li><code>repete(f, n)</code>: excute <code>f()</code> n fois.</li>
    <li><code>prend()</code> ou <code>prend("objet")</code> où "objet" est l'un de <code>"jeton"</code>,
  ...
  <p>Pour créer des robots, on peut utiliser <code>RobotUsage()</code>   </p>
  <p>De plus, les commandes suivantes sont généralement disponibles sans être liées
    à un robot en particulier. </p>
  <ul>
        <li><code>from biblio import *</code>: pour <b>Python</b>, iinclut le code qui se trouve sous l'onglet bibliothèque</li>
    <li><code>examine(objet Javascript)</code></li>
    <li><code>pause()</code>; peut être invoqué avec un argument en millisecondes indiquant la durée de
    la pause dans quel cas le programme continue sans avoir à cliquer sur un bouton. </li>
        <li><code>son("vrai")</code>: ajoute des effets sonores. Ces effets sonores peuvent être arrêtés en utilant "false" comme
                  argument ou omettant l'argument comme ceci: <code>son()</code>.
                  Les effets sonores sont arrêtés par défaut ou si le délai entre les actions est inférieur à 250ms.
                  </li>
    <li><code>pense(nombre)</code>: spécifie le montant de temps (en millisecondes) entre chaque action d'un robot. </li>
    <li><code>ecrit(chaîne)</code>: écrit une chaîne de caractère dans le journal de Reeborg, par exemple <code>ecrit("Bonjour !")</code>,
    ...
    <li><code>selectionne_defi(chaîne)</code>: sélectionne un monde (caché) donné. </li>
        <li><code>print(chaîne)</code>: seulement valide avec Python; semblable à <code>write</code>.</li>

  </ul>
'''
RUR = window.RUR
RUR.ui.add_help(_usage_, "fr")
