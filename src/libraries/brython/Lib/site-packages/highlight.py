...
def insert_highlight_info(src):
    ...
    for lineno, line in enumerate(lines):
        ...
        if first_word in 'pass def class continue break if from import del return raise try with yield'.split():
            ...
            new_lines.append( indent + first_word + tracing_line(' ', current_group, frame=True) + ' and' + remaining)
        elif '=' in line_wo_indent:
            new_lines.append(tracing_line(indent, current_group, frame=True))
            new_lines.append(line)
        elif not first_word and remaining[0] == "#":
            new_lines.append(line)
        elif (first_word in 'narration move turn_left pause think take put build_wall front_is_clear is_facing_north at_goal object_here write global nonlocal print repeat'.split()
             or first_word in 'avance tourne_a_gauche pense prend depose construit_mur rien_devant face_au_nord au_but objet_ici ecrit repete'.split()):
