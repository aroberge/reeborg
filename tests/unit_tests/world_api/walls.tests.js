...
test('Add and get wall at', function (assert) {    
    require("../../../src/js/world_api/wall.js");
    RUR.CURRENT_WORLD = RUR.create_empty_world();
    RUR.add_wall(3, 3, "east");  // west of (4, 3)
    RUR.add_wall(3, 3, "north");  // south of (3, 4)
    assert.ok(RUR.is_wall_at(3, 3, "east"), "east wall present");
    assert.ok(RUR.is_wall_at(3, 3, "north"), "north wall present");
    assert.notOk(RUR.is_wall_at(3, 3, "south"), "south wall not present");
    assert.notOk(RUR.is_wall_at(3, 3, "west"), "west wall not present");
    //
    assert.ok(RUR.is_wall_at(4, 3, "west"), "east wall present");
    assert.ok(RUR.is_wall_at(3, 4, "south"), "south wall present");
    assert.end();
});
